({
  ...
  createCalculation: function(cmp) {
    ...
    this.upsertCalculation(cmp, calculation, function(a) {
      ...
      returnValue.DateSign__c = $A.localizationService.formatDate(
        returnValue.DateSign__c,
        "YYYY-MM-DD"
      );
      returnValue.DateStart__c = $A.localizationService.formatDate(
        returnValue.DateStart__c,
        "YYYY-MM-DD"
      );
  ...
  createFirstInsured: function(cmp, accountType) {
    ...
    console.log("jdu dal");
    this.upsertAccount(cmp, firstInsured, accountType, calc, relation, function(
      a
    ) {
      var returnValue = a.getReturnValue();
      returnValue.PersonBirthdate = $A.localizationService.formatDate(
        returnValue.PersonBirthdate,
        "YYYY-MM-DD"
      );
  ...
  createPolicyHolder: function(cmp, accountType) {
    ...
    var calc = cmp.get("v.NNCalculation");
    this.upsertAccount(cmp, policyHolder, accountType, calc, relation, function(
      a
    ) {
      var returnValue = a.getReturnValue();
      returnValue.PersonBirthdate = $A.localizationService.formatDate(
    ...
    action.setCallback(this, function(response) {
      ...
      if (state === "SUCCESS") {
        ...
        if (s == 2) {
          ...
          component.set(
            "v.secondInsured.NetMonthyIncome__pc",
            parsed.netIncome
          );
        }
      }
  ...
  getComponents: function(component, insuredPersonIndex) {
    ...
    var calculationRequest = component.get(
    ...
    action.setCallback(this, function(response) {
      ...
      if (state === "SUCCESS") {
        ...
        component.set(
          "v.NNCalculation.Group_Discount__c",
          parsed.groupDiscount
        );
        console.log("blabla ");
        component.set(
          "v.NNCalculation.Frequency_Discount__c",
          parsed.frequencyDiscount
        );
        component.set(
          "v.NNCalculation.Calculated_Employer_Payment__c",
          parsed.calculatedEmployer
        );
        component.set(
          "v.NNCalculation.Total_Premium_After_Discount__c",
          parsed.totalAfterDiscount
        );
        component.set(
          "v.NNCalculation.Calculated_Client_Payment__c",
          parsed.calculatedClient
        );
        console.log("bla ");
        component.set(
          "v.NNCalculation.VPMS_Calculation_Request__c",
  ...
  recalculateFunds: function(component) {
    ...
    action.setCallback(this, function(response) {
      ...
      if (state === "SUCCESS") {
        ...
        component.set(
          "v.NNCalculation.Expected_Yield__c",
          parsed.expectedYield
        );
        component.set(
          "v.NNCalculation.Total_Fund_Value__c",
          parsed.totalFundValue
        );
        component.set(
          "v.NNCalculation.Calculated_Client_Payment__c",
          parsed.calculatedClient
  ...
  recalculateRiders: function(component, indexPerson) {
    ...
    var groupDiscount =
      component.get("v.NNCalculation.Group_Discount_Percent__c") != undefined
        ? component.get("v.NNCalculation.Group_Discount_Percent__c")
        : 0;
    var employerContribution =
      component.get("v.NNCalculation.Employer_Contribution__c") != undefined
        ? component.get("v.NNCalculation.Employer_Contribution__c")
        : 0;
    ...
    action.setCallback(this, function(response) {
      ...
      component.set("v.autoCorrectMessages", parsed.autoCorrectMessages);
      component.set(
        "v.NNCalculation.PaymentFrequency__c",
  ...
  getSelectedProduct: function(cmp, productCode) {
    const selectedProductArray = cmp
      .get("v.productOptions")
      .filter(row => row.value == productCode);
    const output =
